...
public:
	...
	*/
	struct Shot {
		...
		*/
		...
		/*
		the number of times the shot can bounce; reduced by 1 every time the shot bounces 
		off a wall and when it hits 0, the shot is deleted.
		*/
		int bounces_remaining_;

		/*
		constructor; creates a shot at the designated x and y position with a trajectory 
		defined by an initial angle.
		*/
	...
	*/
	...
	/*
	checks whether a player is hit by any given shot within the level. If the player gets hit 
	call the kill_player function of the player.
	*/
	...
	*/
	...
	/*
	serialized string format of all shots in the level. Used in multiplayer; shots are managed 
	at the host server and sent to the client.
	*/
	string serialized_string();

	/*
	deserialize an update message from the host server and adds the appropriate shots to the 
	ShotsInLevel in the client program.
	*/
