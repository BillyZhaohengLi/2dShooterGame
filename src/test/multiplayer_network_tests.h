...
inline void MULTIPLAYER_NETWORK_TESTS() {
	...
	MULTIPLAYER_TEST.ASSERT_EQUALS("test initial state", true, test_network.received_message_from_server());

	MultiplayerNetwork test_network2;
	//start a server
	MULTIPLAYER_TEST.ASSERT_EQUALS("test conflicting server setup", false, test_network2.server_setup());

	//the network should still be disconnected since the setup failed
	MULTIPLAYER_TEST.ASSERT_EQUALS("test state after failed server setup", NONE, test_network2.get_status());
	...
	//the network should still be disconnected since the setup failed
	MULTIPLAYER_TEST.ASSERT_EQUALS("test state after failed client setup", NONE, test_network2.get_status());
	...
	MULTIPLAYER_TEST.ASSERT_EQUALS("test state after successful client setup", CLIENT, test_network2.get_status());
	...
	MULTIPLAYER_TEST.ASSERT_EQUALS("host sending to client", std::string("aaa"), test_network2.receive());

	//test client sending to server
	test_network2.send("bbb");
	MULTIPLAYER_TEST.ASSERT_EQUALS("client sending to host", std::string("bbb") , test_network.receive());
	...
	MULTIPLAYER_TEST.ASSERT_EQUALS("test server is_connected with no clients", false, test_network.is_connected());
	...
	MULTIPLAYER_TEST.ASSERT_EQUALS("test manual disconnect by server", false, test_network3.is_connected());
